...
formalsList::formalsList(formalsDecl &formalsDecl, formalsList &formalsList) : Node("formalsVector") {
    ...
    this->formalsVector.insert(this->formalsVector.end(), formalsList.formalsVector.begin(), formalsList.formalsVector.end());
}
...
ClosedStatement::ClosedStatement(string firstKeyWord, exp &exp, ClosedStatement &ClosedStatement) : Node("ClosedStatement") {
    if (firstKeyWord != "WHILE") {
        output::errorSyn(lineno);
        exit(0);
    }
